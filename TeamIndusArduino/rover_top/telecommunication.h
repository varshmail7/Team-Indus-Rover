...
class telecommunication
{
  ...
  public:
    ...
    // Constructor
    telecommunication(int baud = 9600)
    {
      initialize_telecommunication(baud);
    }
...
#ifdef TELECOMMUNICATION_UNITTEST
      displn("TELECOMM :: x : " + String(x));
      displn("TELECOMM :: y : " + String(y));
      delay(2000);
#endif

    }
    // Read Speed
    void read_speed(float &speed)
    {
      speed = Serial.parseFloat();

#ifdef TELECOMMUNICATION_UNITTEST
      displn("TELECOMM :: speed : " + String(speed));
      delay(2000);
#endif
    ...
    {
#ifndef TELECOMMUNICATION_UNITTEST
    ...
    int telecommand(float &x_g, float &y_g, int &mode, float &speed)
    {
